...
const int AMOUNT_OF_SOUND_EFFECTS=7;
...
int main()
{
    ...
    window.create(VideoMode(SCREEN_WIDTH, SCREEN_HEIGHT), "SFML Luigi Demo"); //gives the main window attributes such as width, height, and a name
    //-----------------------------
    ...
    string soundEffectName[AMOUNT_OF_SOUND_EFFECTS]={"lm_coin.wav","okeedokee.wav","SFX_Luigi_crazy-laugh.wav","SFX_Luigi_letsa-go.wav","SFX_Luigi_Whoa.wav","SFX_Luigi_woohoo.wav","SFX_Luigi_owowowowow.wav"};//SoundBuffers need the name of the file, to look for it. Order matters (PUT ALL FILES IN BUILD FOLDER)

    for (int i=0;i<AMOUNT_OF_SOUND_EFFECTS;i++)
    {
        string temp="../resources/";
        temp=temp+soundEffectName[i]; //adds the destination folder path ("../resources/") to the front of the file name.

        if (!buffer[i].loadFromFile(temp)) //ie: load the 1st SoundBuffer using the 1st soundEffectName into the 1st sound
    ...
    if (!music.openFromFile("../resources/Super Mario 64 Soundtrack - Super Mario 64 (Main Theme) (256kbit).wav")) //opens the file (note: doesn't "loadFromFile" since streaming)  (PUT ALL FILES IN BUILD FOLDER)
    {
        cout<<"File failed to load!!!"<<endl;
    }
    ...
    if (!backgroundTexture.loadFromFile("../resources/mario2s1.gif")) //loads file into texture (PUT ALL FILES IN BUILD FOLDER)
    {
        cout<<"File failed to load!!!"<<endl;
    }
    ...
    if (!luigiDanceTexture.loadFromFile("../resources/LuigiDanceSpriteSheet.png")) //loads file into texture (PUT ALL FILES IN BUILD FOLDER)
    {
        cout<<"File failed to load!!!"<<endl;
    }
    ...
    while (window.isOpen())//GAME LOOP
    {
        while (window.pollEvent(event)) //EVENT LOOP
        {
            ...
            else if (event.type==Event::KeyPressed)
            {
                ...
                case Keyboard::C:
                    sound[2].play(); //plays the sound effect in slot 2 (currently the SFX_Luigi_crazy-laugh sound)
                    break;

                case Keyboard::V:
                    sound[3].play(); //plays the sound effect in slot 3 (currently the SFX_Luigi_letsa-go sound)
                    break;

                case Keyboard::B:
                    sound[4].play(); //plays the sound effect in slot 4 (currently the SFX_Luigi_Whoa sound)
                    break;

                case Keyboard::N:
                    sound[5].play(); //plays the sound effect in slot 5 (currently the SFX_Luigi_woohoo sound)
                    break;

                case Keyboard::M:
                    sound[6].play(); //plays the sound effect in slot 6 (currently the SFX_Luigi_owowowowow sound)
                    break;
        ...
        window.draw(luigiDanceSprite); //draws luigi sprite
        window.setView(view); //positions the camera
        window.display(); //displays everything drawn
        system("cls"); //clears the command prompt screen
...
/**
    ...
    @param window: tells function which window to refer to, mouse: gives access to mouse functions, myBox: passes in my box so I can manipulate it
*/
void process(const RenderWindow &window, const Mouse &mouse, RectangleShape &myBox)
{
...
/**
    ...
    @param clock: for starting a timer, time: keeps track of time, sprite: sprite you want to use, speed: speed to play animation at,
           left: position of left side of sprite, top: position of top side of sprite, width: how wide the sprite is,
           height: how tall the sprite is, frames: how many frames your animation has, toggle: for playing animation forwards and backwards repeatedly
*/
void animate(Clock &clock, Time &time, Sprite &sprite,int speed, int &left, int &top, int width, int height, int frames, bool &toggle)
{
